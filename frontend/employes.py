...
# employes.py — Gestion Employés (liste + ajout/modif/suppression)
# Header & navigation alignés sur accueil.py : logo, couleurs, "MAGIC / BOOKS",
# menu hamburger (même structure/ordre/émoticônes), section Compte.
...
# -------- IMPORTS DES MODÈLES (complets pour résoudre les relations) --------
...
def _employee_to_dict(e: Employee) -> dict:
    """Transforme un objet Employee en dict d'affichage (robuste via getattr)."""
    d = {
        "id": getattr(e, "_id", None),
        "prenom": getattr(e, "firstname", "") or "",
        "nom": getattr(e, "lastname", "") or "",
        "birthdate": "",
        "job_title": getattr(e, "_jobTitle", None),  # optionnel si présent dans ton modèle
    }
    bd = getattr(e, "birthdate", None)
    d["birthdate"] = bd.strftime("%Y-%m-%d") if bd else ""
    return d


# ---------- Popup création / modification ----------
class EmployeeOverlayWindow:
    """
    Popup scrollable "Ajouter / Modifier un employé"
    Champs : firstname, lastname, birthdate (héritage Person)
    Option : _jobTitle si présent dans ton modèle (sinon ignoré).
    """
    def __init__(self, parent, on_validate=None, employee_data=None):
        self.parent = parent
    ...
    def show(self):
        ...
        self.win.title("Modifier un employé" if self.is_edit else "Ajouter un employé")
        ...
        if self.is_edit:
            self.entry_id.insert(0, str(self.employee_data.get("id", "")))
        ...
        ctk.CTkLabel(fr_fields, text="POSTE (optionnel) :", font=("Arial", 14, "bold")).grid(
            row=row, column=0, sticky="w", padx=20, pady=8
        )
        self.entry_job = ctk.CTkEntry(fr_fields, width=340, height=40, font=("Arial", 12))
        self.entry_job.grid(row=row, column=1, padx=20, pady=8, sticky="w")
        row += 1

        # Pré-remplissage si édition
        if self.is_edit:
            self.entry_prenom.insert(0, self.employee_data.get("prenom", ""))
            self.entry_nom.insert(0, self.employee_data.get("nom", ""))
            self.entry_birthdate.insert(0, self.employee_data.get("birthdate", ""))
            if self.employee_data.get("job_title"):
                self.entry_job.insert(0, self.employee_data.get("job_title"))
    ...
    def validate(self):
        # Collecte
        data = {
            "id": self.employee_data.get("id") if self.is_edit else None,
            "prenom": self.entry_prenom.get().strip(),
            "nom": self.entry_nom.get().strip(),
            "birthdate": self.entry_birthdate.get().strip(),
            "job_title": self.entry_job.get().strip() or None,
        }
        # Requis
        if not data["prenom"] or not data["nom"] or not data["birthdate"]:
            self._show_error("❌ Les champs Prénom, Nom et Date de naissance sont obligatoires.")
            return
        # Date
        try:
...
class EmployeeDetailsOverlay:
    def __init__(self, parent, emp_dict: dict):
        self.parent = parent
        self.emp_dict = emp_dict or {}

    def show(self):
        self.win = ctk.CTkToplevel(self.parent)
        self.win.title(f"Détails Employé #{self.emp_dict.get('id', '')}")
        ...
        fr_header = ctk.CTkFrame(self.win, fg_color=COLOR_HEADER, corner_radius=15, height=80)
        fr_header.pack(fill="x", padx=20, pady=20)
        title = f"{self.emp_dict.get('prenom','')} {self.emp_dict.get('nom','')}".strip() or "Employé"
        ...
        fields = [
            ("ID", _safe_str(self.emp_dict.get("id"))),
            ("Prénom", _safe_str(self.emp_dict.get("prenom"))),
            ("Nom", _safe_str(self.emp_dict.get("nom"))),
            ("Date de naissance", _safe_str(self.emp_dict.get("birthdate"))),
        ]
        if self.emp_dict.get("job_title"):
            fields.append(("Poste", _safe_str(self.emp_dict.get("job_title"))))
...
# ---------- Fenêtre principale ----------
class EmployesApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Gestion Employés")
        ...
        self.create_scrollable_employees_list()
    ...
    def create_toolbar(self):
        ...
        ctk.CTkButton(
            bar, text="➕ AJOUTER EMPLOYÉ", fg_color="#6366F1", hover_color="#4F46E5",
    ...
    def create_search_bar(self):
        ...
        self.search_entry = ctk.CTkEntry(
            search_frame, placeholder_text="🔍 Rechercher par ID, NOM, PRÉNOM, POSTE",
            height=40, font=("Arial", 12)
        )
        self.search_entry.pack(fill="x")
        self.search_entry.bind("<KeyRelease>", lambda e: self.refresh_employees_list())

    # ---------- Liste d'employés ----------
    def create_scrollable_employees_list(self):
        self.employees_scrollable_frame = ctk.CTkScrollableFrame(
            self, fg_color="#E8E8E8", corner_radius=10
        )
        self.employees_scrollable_frame.pack(fill="both", expand=True, padx=50, pady=(10, 20))
    ...
    def open_employes(self):
        """Déjà sur la page employés → on ne relance pas la page courante."""
        self.hide_menu()
    ...
    def show_menu(self):
        ...
        menu_buttons = [
            ...
            ("💼 Employés",  self.open_employes),  # courant
    ...
    def refresh_employees_list(self):
        # Clear
        for w in self.employees_scrollable_frame.winfo_children():
            w.destroy()
        ...
        try:
            ...
            rows = session.query(
                Employee._id, Employee.firstname, Employee.lastname, Employee.birthdate
            ).order_by(Employee._id.asc()).all()
            print(f"💼 Employés trouvés: {len(rows)}")

            search = self.search_entry.get().lower() if hasattr(self, "search_entry") else ""
            prepared = []
            for _id, fn, ln, bd in rows:
                item = {
                    "id": _id,
                    "prenom": fn or "",
                    "nom": ln or "",
                    "birthdate": bd.strftime("%Y-%m-%d") if bd else "",
                    "job_title": None,
                }
                # Essayer d'afficher un intitulé de poste si présent
                try:
                    e = session.query(Employee).filter(Employee._id == _id).first()
                    jt = getattr(e, "_jobTitle", None)
                    if jt:
                        item["job_title"] = jt
                except Exception:
                    pass
                ...
                if search:
                    if (
                        ...
                        or search in (item["job_title"] or "").lower()
                    ):
            ...
            if not prepared:
                ctk.CTkLabel(
                    self.employees_scrollable_frame, text="Aucun employé trouvé",
                    font=("Arial", 14), text_color="#666666"
                ).pack(pady=50)
            else:
                for item in prepared:
                    self.create_employee_card(item)

        except Exception as e:
            print(f"❌ Erreur lors du chargement des employés: {e}")
            import traceback; traceback.print_exc()
            ctk.CTkLabel(
                self.employees_scrollable_frame, text="Erreur de chargement.",
                font=("Arial", 14), text_color="red"
            ).pack(pady=50)
        finally:
            session.close()

    def create_employee_card(self, emp: dict):
        card_frame = ctk.CTkFrame(
            self.employees_scrollable_frame, fg_color="#F5F5F5",
        ...
        initials = _initials(emp.get("prenom",""), emp.get("nom",""))
        ...
        full_name = f"{emp.get('prenom','')} {emp.get('nom','')}".strip()
        ctk.CTkLabel(row, text=full_name or "Employé (sans nom)", font=("Arial", 14, "bold"),
                     text_color="#333333").pack(side="left")
        if emp.get("job_title"):
            ctk.CTkLabel(row, text=f" — {emp['job_title']}", font=("Arial", 14),
        ...
        if emp.get("birthdate"):
            ...
            ctk.CTkLabel(row, text=f" {emp['birthdate']}", font=("Arial", 12),
                         text_color="#333333").pack(side="left")

        # ID
        row = ctk.CTkFrame(info_frame, fg_color="transparent"); row.pack(fill="x", pady=(2, 0))
        ctk.CTkLabel(row, text="ID Employé :", font=("Arial", 12, "bold"),
                     text_color="#555555").pack(side="left")
        ctk.CTkLabel(row, text=f" #{emp.get('id','')}", font=("Arial", 12),
        ...
        ctk.CTkButton(
            ...
            command=lambda d=emp: EmployeeDetailsOverlay(self, d).show()
        ).pack(pady=5)
        ctk.CTkButton(
            actions, text="✏️ Modifier", width=120, height=35,
            fg_color="#6366F1", hover_color="#4F46E5", font=("Arial", 11, "bold"),
            command=lambda d=emp: self.open_edit_overlay(d)
        ).pack(pady=5)
        ctk.CTkButton(
            actions, text="🗑️ Supprimer", width=120, height=35,
            fg_color=PALE_RED, hover_color=PALE_RED_HOVER, font=("Arial", 11, "bold"),
            command=lambda d=emp: self.delete_employee(d["id"])
        ).pack(pady=5)

    # ---------- CRUD ----------
    def open_create_overlay(self):
        EmployeeOverlayWindow(self, on_validate=self.save_employee).show()

    def open_edit_overlay(self, emp_dict: dict):
        EmployeeOverlayWindow(self, on_validate=self.save_employee, employee_data=emp_dict).show()

    def save_employee(self, data, is_edit=False):
        """
        Insert / Update Employee en base.
        Champs : firstname, lastname, birthdate. Option : _jobTitle si présent.
        """
        session = get_session()
        try:
            if is_edit and data["id"] is not None:
                emp = session.query(Employee).filter_by(_id=data["id"]).first()
                if not emp:
                    self._toast_error(f"Employé ID {data['id']} introuvable.")
                    return
                emp.firstname = data["prenom"]
                emp.lastname = data["nom"]
                emp.birthdate = data["birthdate"]
                # Champ optionnel
                if hasattr(emp, "_jobTitle"):
                    setattr(emp, "_jobTitle", data["job_title"])
                session.commit()
                print(f"✅ Employé modifié : {emp.firstname} {emp.lastname} (ID: {emp._id})")
            else:
                emp = Employee(
                ...
                session.add(emp)
                session.commit()
                print(f"✅ Nouvel employé créé : {emp.firstname} {emp.lastname} (ID: {emp._id})")

            self.refresh_employees_list()

        except Exception as e:
            session.rollback()
            print(f"❌ Erreur save_employee : {e}")
            import traceback; traceback.print_exc()
    ...
    def delete_employee(self, emp_id: int):
        session = get_session()
        try:
            emp = session.query(Employee).filter_by(_id=emp_id).first()
            if not emp:
                self._toast_error("Employé introuvable.")
                return

            # Si tu as des contraintes (ex: emprunts gérés par employé), on peut empêcher la suppression ici.
            # Exemple (commenté, car dépend de ton modèle) :
            # if len(getattr(emp, "borrows_managed", []) or []) > 0:
            #     self._toast_error("Impossible de supprimer : des emprunts sont associés.")
            #     return

            fullname = f"{emp.firstname} {emp.lastname}"
            session.delete(emp)
            session.commit()
            print(f"✅ Employé supprimé : {fullname} (ID: {emp_id})")
            self.refresh_employees_list()

        except Exception as e:
            session.rollback()
            print(f"❌ Erreur delete_employee : {e}")
            import traceback; traceback.print_exc()
